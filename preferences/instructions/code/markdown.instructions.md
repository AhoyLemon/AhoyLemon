---
applyTo: "**/*.md"
description: "No artificial line-wrapping in authored Markdown prose"
---

# Markdown Preferences

Every paragraph of prose is one line. Never insert line breaks to hit a column width — GitHub soft-wraps anyway. Line breaks are for structure only: lists, table rows, code blocks, headings, and intentional `<br>`.

Applies to all authored Markdown: `.md` files, docs, GitHub issue/PR bodies, and commit messages. No exception for the git ~72-column tradition — GitHub soft-wraps those anyway.
